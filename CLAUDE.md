...
2. **Субагент пишет только внутрь своего worktree, и только внутрь репозитория.** Backstop —
   `enforce-subagent-worktree.sh`: пути вне репозитория (соседний проект на Desktop, файл в
   `$HOME`, системный путь) отклоняются; проходит только скрэтч — `/tmp`, `$TMPDIR`, `/dev/null`.
   Через Bash (`sed -i`, `echo >`) эта граница пока не закрыта — это E-005.
...
11. **Сессия действует без пошаговых подтверждений владельца** — берёт батч, диспатчит, ревьюит, мержит. Исключение — необратимые действия наружу (публикация, деплой, секреты, прод-миграция): они остаются поштучными. Обоснование и полный список — [.claude/rules/backlog.md](.claude/rules/backlog.md) § Постоянная авторизация.
