...
describe("vectorOrderingProblem", () => {
  ...
  // Judged per file, the correct set went red the first time anyone added one.
  it("accepts a later migration whose extension was created by an earlier one", () => {
    const sql = 'CREATE TABLE "c" ("e" vector(384));';
    expect(vectorOrderingProblem(sql, true)).toBeNull();
    expect(vectorOrderingProblem(sql, false)).toMatch(/never creates the vector extension/);
  });
