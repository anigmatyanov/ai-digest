...
if (!existsSync(MIGRATIONS)) {
  ...
  const dirs = readdirSync(MIGRATIONS, { withFileTypes: true })
    ...
    .map((e) => e.name)
    .sort();
  // Migrations apply in name order, so the extension created by an earlier one is present
  // for every later one. Carried across the loop rather than re-derived per file.
  ...
  for (const dir of dirs) {
    ...
    const problem = vectorOrderingProblem(body, extensionSeen);
    if (body.includes("CREATE EXTENSION")) extensionSeen = true;
...
console.log("  WHICH Neon project or branch these URLs point at. A connection string from a");
console.log("  neighbouring project passes every check above and reports all green.");
