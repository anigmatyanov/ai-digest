...
export class PrismaRepo implements Repo {
  ...
  async putCandidates(candidates: Candidate[]): Promise<Candidate[]> {
    ...
    for (const c of candidates) {
      ...
      await this.prisma.candidateRawItem.deleteMany({ where: { candidateId: row.id } });
      await this.prisma.candidateRawItem.createMany({
  ...
  async putIssue(issue: Issue): Promise<Issue> {
    ...
    await this.prisma.issueCard.deleteMany({ where: { issueId: row.id } });
    await this.prisma.issueCard.createMany({
